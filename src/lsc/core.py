...
def getExpression(inputs_by_element, element_input_or_output, non_standard_elements_expressions):
    ...
    if e_type in standard_elements:
        ...
        inputs_dict = {f"INPUT_{i+1}": inputs_expressions[i] for i in range(e_info["inputs_number"])}
...
def checkProgramInputsOutputs(program):
            ...
            for t in description["tests"]:
                ...
                for key in ["inputs", "outputs"]:
                    ...
                    if have != should_have:
                        return (
                            False,
                            f"Function {name}: Test {t}: {key} number does not match: have {have}, should have {should_have}",
                        )
    ...
    elements_types_inputs_outputs = {**standard_elements, **non_standard_elements}
    ...
    for f_name, description in program.items():
        ...
        for e_name, value in elements_inputs_outputs.items():
            ...
            if have != should_have:
                return (
                    False,
                    f"Function {f_name}: Element {e_name}: inputs/outputs do not match: have {have}, should have {should_have}",
                )
...
def compile(program, target, requirements):
    ...
    if "tests" in program[target]:
        ...
        for s in ["inputs", "outputs"]:
            list_init_values_list = map(
                lambda i: f"{{{i}}}", [", ".join(map(str, t[s])) for t in program[target]["tests"]]
            )
