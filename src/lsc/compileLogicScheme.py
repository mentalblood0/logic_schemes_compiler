...
def joinPrograms(subpaths, root="."):
    ...
    for sp in subpaths:
        ...
        if not os.path.exists(p):
            ...
            dir_content = list(filter(lambda p: p.endswith(".json") or os.path.isdir(p), os.listdir(p)))
...
def getNonstandardRequirements(target_function_name, target_function_description, available_functions_descriptions):

    direct_requirements = core.getElementsTypes(target_function_description)

    direct_nonstandard_requirements = list(filter(lambda e: not (e in core.standard_elements), direct_requirements))
    ...
    for d_r in direct_nonstandard_requirements:
        ...
        d_r_nonstandard_requirements = getNonstandardRequirements(
            d_r, d_r_description, available_functions_descriptions
        )
...
def compileLogicScheme(input_path, target_function_name, linked_paths, output_path):
    ...
    requirements = getNonstandardRequirements(
        target_function_name, target_function_description, available_functions_descriptions
    )
    requirements.reverse()

    functions_descriptions = {
        **{r_name: available_functions_descriptions[r_name] for r_name in requirements},
